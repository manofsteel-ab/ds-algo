...
class Solution:
    ...
    def _topologicalSort(self, v, stack, visited):
        if visited[v] == 3:  #
            return
        if visited[v] == 2:
            self.isCycle = True
        ...
        for node in self.graph.get(v, []):
            self._topologicalSort(node, stack, visited)
        visited[v] = 3
        stack.append(v)

    def findOrder(self, numCourses: int, prerequisites) -> List[int]:
        ...
        for i in range(numCourses):
            if visited[i] == 1:
                self._topologicalSort(i, stack, visited)
        if self.isCycle:
            return []
        return stack
